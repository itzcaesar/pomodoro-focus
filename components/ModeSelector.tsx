...
export const ModeSelector: React.FC<ModeSelectorProps> = memo(({ currentMode, onSelectMode }) => {
  return (
    <div className="flex gap-2 bg-white/25 dark:bg-black/25 backdrop-blur-md p-1.5 rounded-full relative border border-white/30 dark:border-white/10 mx-auto max-w-md" style={{
      ...
      {Object.values(TimerMode).map((mode) => {
        ...
        return (
          <button
            ...
            className={`
                relative z-10 flex-1 py-2 px-3 sm:px-5 rounded-full text-xs font-semibold tracking-wide transition-all duration-300 transform glass-button whitespace-nowrap
                ${isActive 
